...
{
    throw qpid::messaging::MalformedAddress((boost::format("%1%, character %2% of %3%") % message % current % input).str());
}

bool AddressParser::parse(qpid::messaging::Address& address)
{
    std::string name;
    if (readName(name)) {
